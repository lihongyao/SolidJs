...
SolidJS 的 createResource 有四种 TypeScript 重载：

```ts
// 1. 没有 source，但必传 options（带 initialValue）
createResource(fetcher, options: InitializedResourceOptions) 

// 2. 没有 source，可选 options
createResource(fetcher, options?: ResourceOptions) 

// 3. 有 source，必传 options（带 initialValue）
createResource(source, fetcher, options: InitializedResourceOptions) 

// 4. 有 source，可选 options
createResource(source, fetcher, options?: ResourceOptions) 
```

总结：

- **source 可选**：如果没有 source，资源不会自动追踪依赖；fetcher 会默认接收 true 作为 source。
- **options 可选**：可以提供 { initialValue, name }，也可以不提供。
...
- 类型通常是一个 **getter 函数**：() => S
- 作用：当 source() 的值发生变化时自动重新执行 fetcher。
- 特性：
  - 忽略 null、undefined、false 等“空值”。
  - 如果没有提供 source，fetcher 的 sourceOutput 默认为 true。
...
1. sourceOutput：来自 source() 的值，如果没有 source，就是 true。

2. info.value：上一次资源值（或 initialValue）。

3. info.refetching：如果是 refetch() 调用，这里会传入 refetching 值或 true。

返回值：可以是同步值 T，也可以返回 Promise\<T> 异步值。
...
```ts
{
  ...
  onHydrated?: (value: T | undefined) => void;
}
```

- initialValue：资源加载前的初始值。
- name：调试标识。
- 其他高级选项主要用于 SSR 或缓存。
...
| **名称**     | **作用**                                                     |
| ------------ | ------------------------------------------------------------ |
| Resource\<T> | 读取当前数据的函数：resource()，有 resource.loading、resource.error 属性 |
| mutate       | 手动修改资源值，不触发 fetcher                               |
| refetch      | 手动重新执行 fetcher，可传值通过 info.refetching             |
